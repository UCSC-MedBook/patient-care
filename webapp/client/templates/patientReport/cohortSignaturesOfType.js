Template.cohortSignaturesTypeBox.onCreated(function() {
  ...
  instance.sampleLabels = new ReactiveVar(_.pluck(Template.parentData(1).samples, "sample_label"));
  ...
  instance.autorun(function() {
    ...
    instance.subscribe('topCohortSignatures', typeName, limit, instance.sampleLabels.get(), function() {// callback
      // set chart min/max
      var minimum;
      var maximum;
      ...
      for (var index in list) {
        ...
        if (maximum === undefined || samples[samples.length - 1].value > maximum) {
...
Template.renderChart.rendered = function() {
  ...
  this.autorun(function(first) {
    ...
    var context = {
      ...
      "dom_selector" : data.type + data.algorithm + removeSpaces(data.label),
...
Template.signatureRow.helpers({
  grandparentData : function() {
    return (Template.parentData(2));
  },
  getQuery : function() {
    // build a query string
    var grandparentData = Template.parentData(2);
    var patient_label = grandparentData.patient_label;
    var query = "patient_label=" + patient_label;
    return query;
  }
});
